DexpaApp.controller('AdminCtrl', function ($scope, $http, $filter) {
    ...
    function getUsers() {
        $http.get("/Admin/GetUsers").success(function (result) {
            $scope.users = result;
            $scope.$apply();
        }).error(function (msg) {
    ...
    function ManageUser(UserName) {
        document.location.href = "/Account/Manage?userName=" + UserName;
    }
    ...
    function deleteModalWC(WC) {
        ...
        for (var i = 0; i < $scope.drivers.length; i++) {
            if ($scope.drivers[i].workConditions != null) {
                if (WC.id == $scope.drivers[i].workConditions.id) {
...
DexpaApp.controller('TracksCtrl', function ($scope, $http, $filter) {
    ...
    $scope.selectDriver = function (driver) {
        if (driver == undefined) {
            ...
            $scope.driverQuery = [$scope.selectedDriver.callsign] + " - " + $scope.selectedDriver.lastName + " " + $scope.selectedDriver.firstName + " " + $scope.selectedDriver.middleName;
